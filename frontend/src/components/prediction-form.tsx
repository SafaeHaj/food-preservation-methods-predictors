...
import {
  Plus, X, Loader2, FlaskConical, Milk, Thermometer, PackageOpen, Activity, TestTube2,
  SlidersHorizontal, ClipboardList,
} from "lucide-react";
import { toast } from "sonner";

import { api, BEST_MODEL_KEY, MODEL_LABELS, type SchemaData } from "@/lib/api";
...
/** Turns raw dataset tokens ("sealed_high_barrier") into readable labels
 * ("sealed high barrier") for display. Values sent to the API stay raw. */
function prettify(v: unknown): string {
  if (v === null || v === undefined || v === "") return "—";
  return String(v).replace(/_/g, " ");
}

/** Formatted <SelectValue>: Base UI renders the raw value unless given a
 * formatter function, which is why every Select here passes one. */
...
export function PredictionForm({
  ...
  const { setLastPrediction } = usePredictionStore();
  const [submitting, setSubmitting] = React.useState(false);

  const [model, setModel] = React.useState(BEST_MODEL_KEY);
  ...
  function onFoodMatrixChange(value: string) {
    setFoodMatrix(value);
    const lookup = matrixLookup[value] ?? {};
    setMatrixValues(lookup);
    setMatrixSource({});
  }
  ...
  const requiredMissing =
    ...
    candidates.some((c) => !c.name.trim() || !c.treatment_type || !c.application_method || !c.primary_ingredient_name || c.primary_concentration === null || !c.primary_concentration_unit);
  ...
  const modelLabelOptions = [
    ...modelOptions.map((m) => ({ value: m, label: MODEL_LABELS[m] ?? m })),
    { value: BEST_MODEL_KEY, label: `Best validation model (${MODEL_LABELS[bestModel] ?? bestModel})` },
  ];
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <Card className="surface">
          <CardHeader>
            <SectionHeading
              icon={TestTube2}
              number={5}
              title="Treatment / formulation"
              description="Candidate treatments to compare against the control (1–4)."
            />
          </CardHeader>
      ...
      <div className="lg:sticky lg:top-[88px] lg:self-start">
        <div className="relative">
          {/* Halo — marks this as the page's one call-to-action panel, not just
          ...
          <CardHeader>
            ...
            <CardDescription>Review before predicting.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-xs">
            ...
            <SummaryRow label="Indicator" value={`${indicatorType.replace(/_/g, " ")}, threshold ${indicatorThreshold} ${indicatorUnit}`} />
            <SummaryRow label="Candidates" value={`${candidates.length} treatment${candidates.length > 1 ? "s" : ""}`} />
            <AnimatePresence initial={false}>
              {candidates.map((c) => (
                <motion.div
                  key={c.key}
...
/** Numbered section header used throughout the form: an icon chip carries the
 *  section's identity at a glance, the number keeps the "fill this in order"
 *  sense the plain "N ·" prefix had. */
