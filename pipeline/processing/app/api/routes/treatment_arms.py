...
from shared.db.models import AuditEvent, TreatmentArm, User
from shared.schemas.canonical import TreatmentArmCreate, TreatmentArmOut, TreatmentArmUpdate
...
def _get_or_404(arm_id: int, db: Session, user: User, role: str = "viewer"):
    """Fetch and authorize through the project that owns it."""
    return scoping.require_entity(
        db, TreatmentArm, arm_id, user, label="Treatment arm", required_role=role
    )
...
def list_arms(
    ...
    project_id: Optional[int] = Query(
        None, description="Return every arm in the project in one call."
    ),
    ...
    """List treatment arms, by experiment or across a whole project.

    `project_id` exists so the client can stop fanning out: the treatments page previously
    fetched 500 experiments and then issued one request per experiment to assemble the same
    result, which is 501 round trips for one screen.
    """
...
def get_arm(
    ...
    return _get_or_404(arm_id, db, current_user)
...
def update_arm(
    ...
    arm = _get_or_404(arm_id, db, current_user)
